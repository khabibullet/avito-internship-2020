...
final class MainViewContoller: UIViewController {
    ...
    @objc private func selectionButtonTapped() {
        ...
        if let id = screenContents.getCurrentSelectedOfferId() {
            message = """
            \nВы выбрали услугу
            \"\(screenContents.getOfferTitle(id: id))\"
            """
        }
...
extension MainViewContoller: UICollectionViewDataSource {
    ...
    func collectionView(
        ...
        let attributedText = screenContents.getTitle().attributed(by: HeaderReusableView.attributes)
